...
> **Честно про дублирование:** в сервисе есть два разных обработчика шага "питание" в саге. `SagaNutritionCommandListener` (топик `saga-nutrition-command`) на старте саги создаёт профиль через `KbjuCalculatorService.buildDefaultProfile()` — это **фиксированные 2200 ккал**, не зависящие от веса/роста/возраста пользователя, то есть пока заглушка, а не персональный расчёт. Реальная формула (`NutritionCalculationService`, BMR + TDEE с учётом активности) подключена только во втором обработчике — `NutritionSagaConsumer` (топик `saga.nutrition.calculate`). Какой из путей реально используется оркестратором в проде, зависит от конфигурации Saga-Orchestrator.
